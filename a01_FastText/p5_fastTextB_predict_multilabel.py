...
import numpy as np
from a01_FastText.old_single_label.p5_fastTextB_model import fastTextB as fastText
from aa1_data_util.data_util_zhihu import load_data_predict, load_final_test_data, create_voabulary, \
...
tf.app.flags.DEFINE_string("predict_target_file", "fast_text_checkpoint_multi/zhihu_result_ftB_multilabel.csv",
                           "target file path for final prediction")
tf.app.flags.DEFINE_string("predict_source_file", 'test-zhihu-forpredict-v4only-title.txt',
                           "target file path for final prediction")
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        ...
        for start, end in zip(range(0, number_of_training_data, batch_size),
            ...
            logits = sess.run(fast_text.logits,
