...
tf.app.flags.DEFINE_string("cache_file_h5py", "../data/ieee_zhihu_cup/data.h5",
                           "path of training/validation/test data.")  # ../data/sample_multiple_label.txt
tf.app.flags.DEFINE_string("cache_file_pickle", "../data/ieee_zhihu_cup/vocab_label.pik",
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        # Instantiate Model
        fast_text = fastText(num_classes, FLAGS.learning_rate, FLAGS.batch_size, FLAGS.decay_steps, FLAGS.decay_rate,
                             FLAGS.num_sampled, FLAGS.sentence_len,
        ...
        for epoch in range(curr_epoch, FLAGS.num_epochs):  # range(start,stop,step_size)
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),
                ...
                curr_loss, current_l2_loss, _ = sess.run([fast_text.loss_val, fast_text.l2_losses, fast_text.train_op],
                                                         feed_dict={fast_text.sentence: trainX[start:end],
                                                                    fast_text.labels_l1999: trainY[
                                                                                            start:end]})  # fast_text.labels_l1999:trainY1999[start:end]
                ...
                if counter % 50 == 0:
                    print("Epoch %d\tBatch %d\tTrain Loss:%.3f\tL2 Loss:%.3f" % (epoch, counter, loss / float(counter),
                                                                                 current_l2_loss))  # \tTrain Accuracy:%.3f--->,acc/float(counter)
            ...
            # 4.validation
            print("epoch:", epoch, "validate_every:", FLAGS.validate_every, "validate or not:",
...
def do_eval(sess, fast_text, evalX, evalY, batch_size, vocabulary_index2word_label):  # evalY1999
    ...
    for start, end in zip(range(0, number_examples, batch_size), range(batch_size, number_examples, batch_size)):
        ...
        curr_eval_loss, logit = sess.run([fast_text.loss_val, fast_text.logits],  # curr_eval_acc-->fast_text.accuracy
                                         feed_dict={fast_text.sentence: evalX[start:end], fast_text.labels_l1999: evalY[
                                                                                                                  start:end]})  # ,fast_text.labels_l1999:evalY1999[start:end]
