...
root = '/Users/zhangzhen/gitRepository/text_classification/data/mnist'
...
class TextRNN:
    def __init__(self, num_classes, learning_rate, batch_size, decay_steps, decay_rate, sequence_length,
                 vocab_size, embed_size, is_training, input_size=1,
        ...
        self.input_x = tf.placeholder(tf.float32, [None, self.sequence_length, self.input_size], name="input_x")  # X
        ...
        if not is_training:
            return
        self.predictions = tf.cast(tf.argmax(self.logits, axis=1, name="predictions"), dtype=tf.int32)
        # text
        ...
        # mnist
        correct_prediction = tf.equal(self.predictions, tf.argmax(self.input_y, axis=1, output_type=tf.int32))
    ...
    def inference(self):
        ...
        self.input_x = tf.reshape(self.input_x, [-1, self.sequence_length, 1])
        print(self.input_x)
        ...
        with tf.name_scope(
            ...
            logits = tf.nn.softmax(
                tf.matmul(self.output_rnn_last, self.W_projection) + self.b_projection)  # [batch_size,num_classes]
        return logits
    ...
    def loss(self, l2_lambda=0.0001):
        with tf.name_scope("loss"):
            ...
            loss = tf.reduce_mean(
                -tf.reduce_sum(self.input_y * tf.log(self.logits), reduction_indices=[1]))
            l2_losses = tf.add_n(
        ...
        if self.is_training:  # training
            ...
            loss = tf.reduce_mean(
                ...
                tf.nn.nce_loss(weights=tf.transpose(self.W_projection),
                               # [hidden_size*2, num_classes]--->[num_classes,hidden_size*2]. nce_weights:A `Tensor` of shape `[num_classes, dim].O.K.
                               ...
                               # [batch_size,hidden_size*2] #A `Tensor` of shape `[batch_size, dim]`.  The forward activations of the input network.
...
def main():
    ...
    textRNN = TextRNN(num_classes, learning_rate, batch_size, decay_steps, decay_rate, sequence_length, vocab_size,
                      embed_size, is_training, input_size=input_size)

    # 数据获取
    with tf.Session() as sess:
        merged = tf.summary.merge_all()
        train_writer = tf.summary.FileWriter('/tmp/textrnn_train', sess.graph)
        test_writer = tf.summary.FileWriter('/tmp/textrnn_test')
        sess.run(tf.global_variables_initializer())
        batch_x, batch_y = mnist.train.next_batch(batch_size=batch_size)
        batch_x = np.reshape(batch_x, [-1, sequence_length, input_size])
        for e in range(epoches):
            # mnist.train.epochs_completed = 0
            # mnist.validation.epochs_completed = 0
            for i in range(int((mnist.train.num_examples + 1) / batch_size)):
                # input_x = np.zeros((batch_size, sequence_length))  # [None, self.sequence_length]
                # input_y = np.array([1, 0, 1, 1, 1, 2, 1, 1])  # [None,]
                # batch_x, batch_y = mnist.train.next_batch(batch_size=batch_size)
                summary, loss, acc, _, _ = sess.run(
                    [merged, textRNN.loss_val, textRNN.accuracy, textRNN.predictions, textRNN.train_op],
                    feed_dict={textRNN.input_x: batch_x, textRNN.input_y: batch_y,
                               textRNN.dropout_keep_prob: dropout_keep_prob})
                steps = e * (mnist.train.num_examples + 1) / batch_size + i
                train_writer.add_summary(summary, int(steps))
                print("Epoch {} | Step {} | Training loss={} | acc={}".format(e, i, loss, acc))
                # print("Epoch {} | Step {} | Validing loss={} | acc={}".format(e, i, v_loss, v_acc))
                # if i % 100 == 0:
                #     _summary, _loss, _acc, = sess.run([merged, textRNN.loss_val, textRNN.accuracy],
                #                                       feed_dict={textRNN.input_x: mnist.test.images[:2000],
                #                                                  textRNN.input_y: mnist.test.labels[:2000],
                #                                                  textRNN.dropout_keep_prob: 1.0})
                #     _steps = e * (mnist.train.num_examples + 1) / batch_size + i
                #     test_writer.add_summary(summary, int(_steps))
                #     print("Epoch {} | step {} | Testing loss={} | acc={}".format(e, i, _loss, _acc))
